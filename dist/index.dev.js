(function (global, factory) {
  ...
  const TimelineLite = _gsScope.TimelineLite;
  ...
  const TimelineMax = _gsScope.TimelineMax;
  ...
  const TweenMax = _gsScope.TweenMax;
  ...
  const CSSPlugin = _gsScope.CSSPlugin;
   ...
   * @author: Jack Doyle, jack@greensock.com
   **/
  const RoundPropsPlugin = _gsScope._gsDefine.plugin({
    propName: "roundProps",
    version: "1.7.0",
    priority: -1,
    API: 2,
   ...
   * @author: Jack Doyle, jack@greensock.com
   **/
  const DirectionalRotationPlugin = _gsScope._gsDefine.plugin({
  ...
  const Back$1 = _gsScope.Back;
  const Elastic$1 = _gsScope.Elastic;
  const Bounce = _gsScope.Bounce;
  const RoughEase = _gsScope.RoughEase;
  const SlowMo = _gsScope.SlowMo;
  const SteppedEase = _gsScope.SteppedEase;
  const Circ = _gsScope.Circ;
  const Expo = _gsScope.Expo;
  const Sine = _gsScope.Sine;
  const ExpoScaleEase = _gsScope.ExpoScaleEase;
  ...
  const TweenMax$1 = TweenMax;
  TweenMax$1._autoActivated = [TimelineLite, TimelineMax, CSSPlugin, AttrPlugin, BezierPlugin, RoundPropsPlugin, DirectionalRotationPlugin, Back$1, Elastic$1, Bounce, RoughEase, SlowMo, SteppedEase, Circ, Expo, Sine, ExpoScaleEase];
      ...
      switch (type) {
        ...
        case 'type-4':
          nodes.forEach(e => {
            ...
            e.style.lineHeight = 1; // e.style.letterSpacing = '10px'
          });
          nodes.forEach(e => {
        ...
        case 'type-5':
          tl.staggerFrom(nodes, 0.2 * d, {
            opacity: 0,
            scaleX: -1,
            cycle: {
              x: [-100 * this.ratio, -250 * this.ratio]
            }
          }, 0.05 * d);
          break;

        case 'type-6':
          tl.staggerFrom(nodes, 0.02 * d, {
            opacity: 0
          }, 0);
          break;
      }
